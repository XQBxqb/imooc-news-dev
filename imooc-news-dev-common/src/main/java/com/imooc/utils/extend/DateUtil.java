...
public class DateUtil {
    /*常用ISO_EXPANDED_DATE_FORMAT，DATETIME_PATTERN
    * DateUtil.getCurrentDateString(DateUtil.DATETIME_PATTERN):用来表示当前时间，直观得很
    * datoToStringWithTime(new Date())
    * stringToDate（Stirng , Pattern）；
    ...
    * */
    ...
    protected static final float normalizedJulian(float JD) {
        ...
        float f = Math.round(JD + 0.5f) - 0.5f;//Math.round ： float四舍五入----这里你可以举例子，就是把一个浮点数变成省去小数点后再加0.5---  3.* -> 3.5  ,4.*-> 4.5

        return f;
    }

    /**
     * Returns the Date from a julian. The Julian date will be converted to noon GMT,     -----儒略日 (Julian Date)
    ...
    public static final Date toDate(float JD) {
        ...
        float Z = (normalizedJulian(JD)) + 0.5f;//举例就行: 3.*->4, 4.*->5  ...
